...
/*
 * Minimal stroke-icon set (24x24 viewBox, currentColor) so the app needs no
 * emoji or icon dependency. Size/color via className, e.g.
 * <PawIcon className="h-5 w-5 text-primary-600" />.
 */
...
function IconBase({ className = "h-5 w-5", children }: IconProps & { children: ReactNode }) {
  return (
    <svg
...
// Added with the clinic directory: a contact block needs to distinguish a
// phone number from an address from a website at a glance, and the set above
// had no way to say any of the three.
